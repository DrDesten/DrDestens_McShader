...
vec3 mixLightmap(vec3 lightmap /* lmcoord (xy) + AO (z) */, vec3 skyLight, vec3 blockLight) {
    ...
    skyLight   = mix(skyLight,   skyLight / 0.25,  nightVision * (1 - lightmap.y));
    blockLight = mix(blockLight, blockLight / 0.1, nightVision * (1 - lightmap.y));
...
vec3 getCustomLightmap(vec3 lightmap /* lmcoord + AO */, float customLightmapBlend, vec3 blocklightExtraColor) {
    #ifndef LIGHTMAP_COMPLEX_BLOCKLIGHT
