...
float AmbientOcclusionLOW(vec3 screenPos, vec3 normal, float size) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        scoord     *= sign(dot(normal, scoord));                        // Inverts the scoord position if its pointing towards the surface (thus being within it). Much more efficient than using a tbn
